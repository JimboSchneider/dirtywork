...
2. tests/test_runner.py line 348 currently passes a content string that is the opening XML tag, then {}, then the closing tag, written literally. Change it to `_resp(content=TOOL_CALL_OPEN + "{}" + TOOL_CALL_CLOSE)` and add `from .markers import TOOL_CALL_OPEN, TOOL_CALL_CLOSE` next to the file's existing relative helper import (`from .provider_doubles import …`, line ~47) — tests/ is a package, so helpers are imported relatively, never as bare module names.

3. tests/test_soak_tools.py line 471 is one string with the Devstral token embedded: `long_tool = "resultofpreviouscall[…token…]" + ("bash" * 40)`. Split it so the token comes from the helper: `long_tool = "resultofpreviouscall" + TOOL_CALLS + ("bash" * 40)`, add `from .markers import TOOL_CALLS` next to that file's relative imports, and reword the comment two lines above (line 469, which also spells the token) to say "plus a stray Devstral marker + bash" without the token.

4. Self-check without spelling a whole tag: `grep -rn 'tool_call>' tests/*.py` and `grep -rn 'TOOL_CALLS\]' tests/*.py` must each return hits ONLY in tests/markers.py (its concatenated pieces) and nowhere else. Then run `env -u GIT_DIR -u GIT_WORK_TREE python3 -m pytest -q -p no:cacheprovider tests/test_runner.py tests/test_soak_tools.py` and then the whole suite the same way (drop the file arguments). Call finish(summary=...) naming the three files. Do not touch docs/.
```
...
1. dirtywork/toolspec.py, near the top (after the imports, before the first class): copy the comment block from dirtywork/runner.py lines 91-94 here (it starts "These tags are built by concatenation ON PURPOSE") and define, in this exact concatenated form:
...
2. dirtywork/runner.py line 100: delete the `_TEXT_TOOL_MARKERS = tuple("<" + m for m in (...))` line; shorten the comment block at lines 91-94 to one line that still covers `_THINK_OPEN`/`_THINK_CLOSE` right below it (`# Concatenated ON PURPOSE — see TOOL_CALL_MARKERS in toolspec.py`); and add `from .toolspec import TOOL_CALL_MARKERS as _TEXT_TOOL_MARKERS` as a new line right after `from .tools import is_timeout_result, net_change, parse_exit_code` (line 20) — runner.py has no toolspec import today. classify_text_reply (line ~246) keeps using `_TEXT_TOOL_MARKERS` unchanged, so a text-only reply containing Devstral's token now classifies as "text_tool_call". Check there is no import cycle: toolspec must not import runner (it does not today; do not add one).
...
5. tests/test_toolspec.py — add, importing `from .markers import TOOL_CALLS, TOOL_CALL_OPEN`. For tests a–h the registry must have `bash` and `read_file` registered, so build it with `from dirtywork.builtin_tools import default_registry` and `r = default_registry()` (the file's own `_registry()` helper at line ~183 registers neither); test i may use `_registry()` or `default_registry()`:
...
5. tests/test_runner.py — import TOOL_CALLS from markers; use the file's FakeProvider/_resp/_call/_events helpers and the `parts` fixture exactly as neighbouring tests do. Build a polluted call with `_call("c1", "exit code: 0\\n1,User1" + TOOL_CALLS + "bash", {"command": "echo hi"})` (the name is the polluted string; _call accepts any name).
   All six use the `parts` fixture (a plain host sandbox on a non-git directory, where the change guard is off — a single finish completes the run). `result = r.run("s", "t")` returns the object whose `.status` and `.final_message` these tests assert; `run_end` events carry no final_message.
   a. test_polluted_bash_name_is_recovered_and_dispatched: script [polluted bash call, then a finish call]; after the run the tool_result events contain one with tool == "bash" whose result contains "hi" (the command ran) and whose tool_raw == the polluted name; result.status == "completed".
   b. test_three_polluted_calls_in_a_row_do_not_abort: three polluted bash calls on three turns (each its own _resp), then a finish → result.status == "completed" and "consecutive" not in (result.final_message or "").
   c. test_unrecovered_polluted_name_still_strikes_unknown_tool: name = "x" + TOOL_CALLS + "nope"; three such calls → result.status == "model_error" and result.final_message == "aborted after 3 consecutive unknown_tool failures"; the tool_result events for them have NO tool_raw key and their tool == the name (short enough to pass cap_name unchanged).
   d. test_recorded_names_are_capped_head_and_tail: an unrecovered name of about 1000 chars ("p" * 950 + TOOL_CALLS + "zzzz"), then `_resp(content="done")` to end the run (a plain answer completes under `parts`) → the assistant event's tool_calls[0]["name"] has length 201 and contains "…"; that tool_result's tool equals cap_name(name); the run_end event's last_tool_result["tool"] == cap_name(name).
...
6. tests/test_runner.py (`from .markers import TOOL_CALLS`; polluted call = `_call("c1", "exit code: 0" + TOOL_CALLS + "bash", {"command": "echo hi"})`). Fixtures: tests a, b, c, f use `parts` (guard off, one finish completes); tests d, e, g use `git_parts` (a real git worktree with the change guard active — copy the setup of test_zero_change_finish_rejected_once_then_completed, ~line 3223). `result = r.run(...)` carries `.status`/`.final_message`.
   ...
   c. test_recovered_name_nudge_orders_after_timeout: a turn with a polluted bash call whose command times out (copy exactly how the file's existing timeout tests script a timed-out bash command — grep "timed_out" in the file), then `_resp(content="done")` to end the run → that turn's last tool_result's follow_up contains the timeout text before the name_recovered text; the nudge events for the turn are ["timeout", "name_recovered"] in transcript order.
   d. test_recovered_finish_that_completes_writes_no_nudge (`git_parts`): a write_file turn first (so the guard accepts the finish), then a response whose only call is `_call("f", "prose" + TOOL_CALLS + "finish", {"summary": "done"})` → result.status == "completed", no nudge event of kind name_recovered, and the finish's tool_result has tool == "finish" and tool_raw == the polluted name.
   e. test_recovered_finish_refused_by_the_change_guard_gets_the_nudge (`git_parts`): a fresh run whose first response is the polluted finish with no prior change → the first finish is refused: its tool_result `result` is `UNCHANGED_PLAIN` (import it from dirtywork.changes) and that same tool_result carries a follow_up containing "The harness ran `finish`", with a name_recovered nudge event via "tool_result"; a second clean finish then completes the run.
   f. test_three_polluted_calls_over_three_turns_nudge_each_turn (`parts`): three turns each with one polluted call → three name_recovered nudge events, one per turn, and result.status == "completed" after a finish.
   g. test_recovered_finish_on_a_verify_feedback_turn_gets_the_nudge (`git_parts`): build the Runner with `verify="echo boom; exit 3", verify_rounds=1` the way the file's verify-feedback tests do (~lines 1585-1625); a write_file turn, then the polluted finish → the finish's tool_result `result` is the verify feedback and its follow_up contains "The harness ran `finish`"; then a clean finish (with verify_rounds exhausted the run ends verify_failed or completed — assert whatever the neighbouring verify tests assert for a second round, and that exactly one name_recovered nudge exists).
...
1. dirtywork/bench.py line ~48: NUDGE_KINDS gains "name_recovered" APPENDED AT THE END (10 → 11 entries). EMPTY_REPLY_NUDGE_KINDS is unchanged (this nudge records no failure). The summarize legend derives from NUDGE_KINDS — verify it prints eleven names.
   tests/test_bench.py: the pin at ~lines 854-855 `NUDGE_KINDS[-2:] == ("no_change", "unchanged_finish")` becomes `NUDGE_KINDS[-3:] == ("no_change", "unchanged_finish", "name_recovered")`; ~858 `len == 10` → 11; the test named ..._ten_kinds is renamed ..._eleven_kinds and its exact-cell assertion (~line 882) widens from "0/0/0/0/0/0/3/0/0/0" to "0/0/0/0/0/0/3/0/0/0/0" (it splits the s1 detail row into cells and compares the nudges cell exactly — keep that form).
...
5. tests/test_transcript_schema.py: add "name_recovered" to the NUDGE_KINDS list at line ~20. test_a_real_run_emits_the_documented_events (line ~78) runs a real Runner driven by `_NudgingProvider.reply` (lines ~69-75), which returns wire bodies via `tool_call_body(...)` / `text_body(...)` from `.provider_doubles` — this file has no `_call`/`_resp`. Add one branch to `_NudgingProvider.reply` (keep the existing ones and the final `text_body("done")`): `if self.calls == <the next unused count>: return tool_call_body("prose " + TOOL_CALLS + "bash", {"command": "echo hi"}, call_id="cX")` with `from .markers import TOOL_CALLS`, so the run emits a tool_result with tool_raw and a name_recovered nudge; then add assertions that "tool_raw" and "name_recovered" are in _doc_tokens(). The test's existing per-key documented-token loop (~lines 98-103) already checks every key of every tool_result, so that event is covered once the doc rows exist.
