...
# The multi-arch index digest of ghcr.io/jimboschneider/dirtywork-worker:0.6
# as published by release v0.6.0 (publish-image.yml) -- verified via
# `docker pull ghcr.io/jimboschneider/dirtywork-worker:0.6` followed by
# `docker image inspect --format '{{json .RepoDigests}}' ...` and independently
# via `docker buildx imagetools inspect ...`, which agree on exactly this
# digest. This only pins a REGISTRY digest -- resolve_image() enforces it
# against a *pulled* DEFAULT_IMAGE only; a locally built/loaded image warns
# instead of refusing (see resolve_image's docstring), and a user-supplied
# --image is never checked against it at all (docker/README.md documents the
# full procedure). MUST be updated to the new digest whenever the :0.6 tag is
# re-pushed (a manual rebuild/republish) -- otherwise resolve_image() refuses
# every pulled default image with the now-stale pin. (0.5.x pinned :0.5 at
...
PINNED_DIGEST: str | None = "sha256:1f7b98898001b7064d8db396a8a5a1a324df4ce48692597fcd4381ea90e4354a"
