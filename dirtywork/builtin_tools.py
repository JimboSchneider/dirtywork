...
BASH_SPEC = ToolSpec(
    ...
    description="Run a shell command in the worktree (cwd is the worktree "
                ...
                "processes are killed when the command returns. In docker mode you are told which; if they cannot be killed, or the container runs out of memory, it is reset: the working tree survives, but git state you created inside the sandbox (index changes, stashes, local commits) does not, and you are told when that happens.",
    params={
