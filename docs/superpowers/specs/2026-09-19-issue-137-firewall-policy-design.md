...
`decide` never raises, never returns `ALLOW` from step 3, and never lets a worker input become `ALLOW` through exception handling (parent design §11, §19). The detail on an internal error names the exception class and nothing else: no message text, since an exception message can carry worker bytes. Three tiers of evidence, in order of preference: action-stage when an action exists, request-stage otherwise, none only when the event code itself is what failed.
