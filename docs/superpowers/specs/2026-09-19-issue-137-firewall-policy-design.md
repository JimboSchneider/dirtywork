...
| 2 | target is `outside`: absolute, or a leading `..`. In host mode an absolute path that equals a context root or begins with a root followed by `/` is inside; the remainder after the root is resolved lexically with `posixpath.normpath` and goes through the helper, so `/wt/.git/config` and `/wt/src/../.git/config` written are `repo_metadata_target` and `/wt/../x` and `/wt/a/../../etc` are outside. Every other `outside` path is denied. | `path_outside_workspace` or `repo_metadata_target` | `authority` | both, with the host exception |
...
3. guards the two stages separately, because what evidence can survive depends on how far the request got. Any `Exception` (a `FirewallInternalError` from a constructor or an unknown state, or anything else, since a bug is exactly what this guard is for) becomes `PolicyDecision(DENY, FIREWALL_INTERNAL_ERROR, "firewall internal error: <exception class name>")`. If step 1 raised, no action exists: `action None`, `dropped_keys 0`, and the event is built from a synthetic `Rejection(FIREWALL_INTERNAL_ERROR, <same detail>)` through `from_rejection(request, ...)` inside its own guard. If step 2 raised, an action exists and is kept: the outcome carries it and its `dropped_keys`, and the event is built with `from_action(action, policy)`, so it is action-stage with the action's kind, capabilities, semantic status and `action_identity`, and the Supervisor's exact-equivalent key still distinguishes a failure on `a.txt` from one on `b.txt` (review of the first reference found both collapsing to one request-stage identity with no capabilities, which #135 §9.2 reserves for requests that never became actions). If `from_action` raises as well, the request-stage synthetic rejection is the fallback; if that raises too, `event` is `None` and the detail gains the suffix `"; no event"`. The guards catch `Exception`, not `BaseException`, so `KeyboardInterrupt` and `SystemExit` still stop the run.

`decide` never raises, never returns `ALLOW` from step 3, and never lets a worker input become `ALLOW` through exception handling (parent design §11, §19). The detail on an internal error names the exception class and nothing else: no message text, since an exception message can carry worker bytes; and it is cut to `MAX_DETAIL_CHARS` before any `PolicyDecision` or `Rejection` is constructed, with room kept for the `"; no event"` suffix, because a class name long enough to overflow the bound would otherwise make the constructor raise outside the guard (found by the review verifier; a generated 300-character exception class is the test). Three tiers of evidence, in order of preference: action-stage when an action exists, request-stage otherwise, none only when the event code itself is what failed.

`decide_batch(requests, context)` first computes `duplicate_positions(requests)`, a new `normalize.py` helper that returns the batch indexes whose `str` `call_id` repeats an earlier one (never raises; `canonicalize_batch` is refactored onto it so the two agree by construction), under a guard that turns any failure there into an internal-error outcome for every request. Then each request is handled under its own guards, the same two-stage split as `decide`: a duplicate position becomes a `DENY` with `call_id_duplicate` and a request-stage event; every other request goes through `canonicalize` and the same steps as `decide`, so an evaluation failure in the batch keeps its action and its action-stage evidence too. Review of the first reference found the batch normalized as one guarded operation, so one request's normalization failure denied its neighbours; normalizing per request under the per-request guard is what makes the isolation promise true. Every request gets an `Outcome`, in order.
...
8. **`Outcome.event` may be `None` on a triple failure.** Denying is the invariant; the event is evidence, and a harness bug in the event code must not turn into a raise from the one function that promises never to raise.
...
- `decide`: accept path (action, ALLOW, event stage `action`), rejection path (event stage `request`, `action None`), the canonicalization failure with `canonicalize` monkeypatched to raise `RuntimeError` (`DENY`, `firewall_internal_error`, detail naming the class only, `action None`, event stage `request`), the evaluation failure with `evaluate` monkeypatched to raise (same decision and detail, but the canonical action retained, event stage `action` with the action's capabilities and identity, different paths giving different identities and the same path the same one, `dropped_keys` propagated), the double failure with `FirewallEvent.from_action` monkeypatched to raise as well (request-stage fallback), and the triple failure with `from_rejection` raising too (`event None`, detail suffix `"; no event"`); never `ALLOW` from any of these;
- `decide_batch`: mixed batch of allow, deny, rejection and duplicate, in order; a normalization failure injected on the middle request leaves both neighbours unaffected, and an evaluation failure there keeps that request's action and action-stage event; duplicates still detected; a failure in `duplicate_positions` denies every request;
