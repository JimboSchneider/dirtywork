...
So a model that keeps calling `read_files` produces one identity, and one that keeps sending a 6 MiB `write_file` produces another. The raw name is hashed, never emitted. This key is deliberately coarse: a rejected request has no canonical target, and reading one out of its arguments would be a second, untrusted normalization. Two oversized writes to different files therefore share an identity, which is why request-stage events never enter exact-equivalent tracking (section 9.2); the identity is an audit and transcript-dedup key only.
...
**Supervisor counters.** Both stages are DENY evidence, but they feed different counters. An action-stage DENY counts toward total denial volume and toward exact-equivalent tracking under `reason_code + capability_set + action_identity`. A request-stage rejection counts toward total denial volume **only**: its identity cannot distinguish targets (section 9.1), so treating it as exact-equivalent would let oversized writes to different files trip a repeated-attempt rule. Issue #126 keys exact-equivalent tracking on `stage == "action"` events and must not key it on request-stage ones. Request-stage events contribute nothing to the `semantic_unknown` aggregate. Whether the Supervisor weights the `malformed`/`bounds` classes differently from `authority` is issue #126's decision; `reason_class` is on the event so it can, without re-deriving anything.
