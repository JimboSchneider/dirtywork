...
`canonicalize_batch(requests)` walks the batch in order. Only `str` ids take part in duplicate detection: the first request carrying a given string `call_id` (exact string equality on the id as given, before any validation) goes through `canonicalize`; every later request with an equal string `call_id` receives `Rejection(call_id_duplicate)` with `detail` giving its batch index, and is not canonicalized. A non-string id is never a duplicate and never compared: it goes through `canonicalize` like any other request and `check_request` rejects it as `call_id_invalid`. Comparing arbitrary values would let two deeply nested list ids recurse on Python 3.9 and take the whole batch down with them (found in review of the W3 PR), and would report the second of two equal malformed ids as a duplicate rather than as invalid. All other requests are independent: a rejection never affects its neighbours (parent design §12). `batch_size` bounds stay in `check_request`, and consistency between each request's `batch_size` and `len(requests)` is the adapter's responsibility in #138, not checked here.
...
- `recover_name` against the registry's own `recover_name` on a fixture of registry-shaped names, every marker string in the test built by concatenation (`"<" + "tool_call>"`) as the module and the registry do, because `<tool_call>` is a special token of the worker's chat template and a worker cannot write it literally through its own tool channel (plain, each marker, each sanitised marker, whitespace padding, nested markers, a marker with no valid tail, a valid tail with no marker) and on pathological repeated-marker strings with and without a valid tail, asserting equal `(name, marker, cut)`; a one-mebibyte marker-only name completes in under one second; the marker-tuple equality test of section 3;
