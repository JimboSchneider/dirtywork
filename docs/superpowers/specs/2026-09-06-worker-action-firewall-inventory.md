...
| At the audited baseline, Docker listing/search paths can become `find` expressions or `rg` options, allowing side effects beyond advertised read-only intent without bash guardrails. | Concrete operand fix: [PR #145](https://github.com/JimboSchneider/dirtywork/pull/145) (merged), and issue #146's `./` anchor in `_rel` for the bare `-` remainder (`head`/`rg`/`grep`/`stat` read stdin, `cd -- -` changes to `$OLDPWD`). #139 must still reconcile canonical target handling with executor argv semantics; #142 needs policy replay cases proving the boundary. |
