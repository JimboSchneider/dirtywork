...
**Files:**
- Modify: `dirtywork/firewall/normalize.py` (add `Sequence` to the `typing` import, and append `canonicalize_batch` after its last line, as one `edit_file` anchored on that line), `dirtywork/firewall/__init__.py:12-13` and `:46-48` (imports and `__all__`), `tests/test_firewall_normalize.py` (append the batch tests after its last two lines), `tests/test_firewall_request.py:295-296` (the `__all__` pin grows to 40 names)
...
- [ ] **Launch** the brief below verbatim through the invocation in the header, sampler on, model loaded in the same command. Six `edit_file` calls and one `write_file`; an `apply_edits` in place of several `edit_file`s is fine.
