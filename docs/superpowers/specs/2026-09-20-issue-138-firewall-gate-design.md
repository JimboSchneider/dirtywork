...
`outcome.event` is `None` only when both event factories failed: `FirewallEvent.from_action` raised and the request-stage fallback `FirewallEvent.from_rejection` raised too (the #137 design's double and triple failure cases). The record is then written anyway, without retrying either factory and without inventing an action identity:
...
1. **The Firewall owns request-stage faults the registry used to answer.** Unknown tools, undecodable and mistyped arguments and oversize payloads are now denied by the gate before the registry sees them, with the same strike kinds and equivalent text. The registry's own paths for those faults remain reachable only through direct `registry.execute` callers and tests.
...
7. **Default context for direct Runner construction.** The alternative, a required argument, touches all 189 `Runner(...)` sites; the default plus the 20 explicit sites of §3 touches 20.
...
- `execution_args` for every `ActionKind` except `finish`: the dict equals what `toolspec._validate_args` returns for the same raw call (the parity corpus reused), `edits` is a list of dicts, `glob` is `None` when absent, the result carries no unexpected key.
...
- missing event: `FirewallEvent.from_action` and `FirewallEvent.from_rejection` both patched to raise; the `event_missing` record lands with the call's turn and id, and the denial text ends with `; no event`. A second case patches `from_action` only and asserts the tier-2 request-stage event, with no `event_missing` key.
