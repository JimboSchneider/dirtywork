...
@pytest.mark.parametrize("content,finish_reason,expected", [
    ...
    (None, None, "empty"),
    ("   \n", None, "empty"),
    ("<think>let me reason</think>", None, "empty"),
    ("<think>never closed the tag", None, "empty"),
    ("<think>plan</think>Done, wrote the file.", None, "answer"),
    ("<tool_call>{\"name\":\"bash\"}</tool_call>", None, "text_tool_call"),
    ("<function=read_file>{\"path\":\"x\"}</function>", None, "text_tool_call"),
    ("<|tool_call|>bash", None, "text_tool_call"),
    ("<function_call>bash</function_call>", None, "text_tool_call"),
...
def test_think_only_reply_is_nudged(parts):
    wt, executor, transcript, tmp = parts
    client = FakeClient([_resp(content="<think>hmm</think>"), _resp(content="ok")])
    r = Runner(client, executor, transcript, model="m")
    result = r.run("s", "t")
    transcript.close()
...
def test_text_tool_call_reply_is_nudged(parts):
    wt, executor, transcript, tmp = parts
    client = FakeClient([_resp(content='<tool_call>{"name":"bash","arguments":{}}</tool_call>'),
                         _resp(content="ok")])
    r = Runner(client, executor, transcript, model="m")
    result = r.run("s", "t")
    transcript.close()
...
_TEXT_TOOL_MARKERS = ("<tool_call>", "<function=", "<function_call>", "<|tool_call|>")
...
def strip_think(text) -> str:
    """Drop every <think>…</think> block (an unterminated <think> drops to the end)."""
