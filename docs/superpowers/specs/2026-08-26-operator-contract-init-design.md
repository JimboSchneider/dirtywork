...
v3.1, 2026-08-26. Status: **red-team folded; release decision taken (0.12.0 minor with the image cycle); awaiting owner sign-off.** Next: plan → built the dogfood way (released dirtywork + local worker for the code; Claude writes the skill prose and reviews).
...
| A9 | Image cycle consistent | `DEFAULT_IMAGE`, `ci.yml`'s docker-live tag, `tests/test_docker_args.py`, the packaged contract and `docker/README.md` all name the same `:0.12` tag; `PINNED_DIGEST is None`; the repo-wide grep for `dirtywork-worker:0.11` outside `docs/superpowers/` and the digest-history comment returns nothing |
