...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1092 passed, 1 skipped, 18 deselected` (1081 + 11).
...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1102 passed, 1 skipped, 18 deselected` (1092 + 10).
...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1108 passed, 1 skipped, 18 deselected` (1102 + 6).
...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1130 passed, 1 skipped, 20 deselected` (1108 + 22 passed; 18 + 2 deselected, the two `tests/test_live_ollama.py` tests).
...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1145 passed, 1 skipped, 20 deselected` (1130 + 15: `tests/test_rundir.py` contributes 8 — one direct test plus the seven parametrized slug cases — `tests/test_main.py` 1, and `tests/test_workspace.py` 6).
...
Run: `/usr/bin/python3 -m pytest -q`
Expected: exit code 0; `1148 passed, 1 skipped, 20 deselected` (1145 + 3).
...
| §4.3 two-pass hashing: pass 1 without `-w` for files and symlink targets, compared against `ls-tree -r -z` with the same normalization; equal → `None`; different → re-run with `-w` and rebuild from the second pass (execution amendment, 2026-08-23: the original "no loose objects on a no-op" framing was imprecise — git's content-addressed store already made `-w` a no-op on unchanged content pre-change, so that half of the claim held before this task too; the change's real, measured effect is a genuine no-op now skipping the temp index + `write-tree` and no longer freshening blob mtimes (~21% faster at 20k files), against a second full read+hash on the changed path (+9-19%). Task 10's fix round 1 renamed the pinning test to check the command transcript, not the object count.) | Task 10, Steps 11 and 12; tests `test_snapshot_worktree_makes_a_no_op_decision_without_writing` (renamed in the fix round; checks the `hash-object`/`update-index`/`write-tree` transcript, object count kept as a secondary invariant), `test_snapshot_worktree_tree_only_references_written_blobs` |
