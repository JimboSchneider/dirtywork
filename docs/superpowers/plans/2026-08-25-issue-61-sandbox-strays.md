...
6. test_docker_live_root_gitfile_tampering_20a: bash "rm .git; git status >/dev/null 2>&1; echo rc=$?" then a bash command that kills the container's tether from inside (`for p in /proc/[0-9]*; do read -r c 2>/dev/null < "$p/comm" || continue; [ "$c" = cat ] && kill -9 "${p#/proc/}"; done; echo killed` — the container dies, the next `docker top` fails, the harness resets as "container unreachable after bash"; a fork bomb would NOT do: the fork-free kill survives pid saturation, spec §1.5) then bash "cat .git" then finish: the first result contains "rc=128"; a sandbox_reset event with reason "container unreachable after bash" exists; the last result contains "gitdir: /gitdir".
...
2. test_docker_live_pid_flood_past_limit_recovers_or_fails_closed (existing): extend its assertions — when the transcript has a sandbox_reset event it carries a non-empty "strays" list of strings; when it has a stray_kill instead (the fork-free kill survives pid saturation) that event's "strays" is non-empty. (test_docker_live_process_flood_is_killed_in_place already covers the 40-process case: one stray_kill, no reset — leave it.)
